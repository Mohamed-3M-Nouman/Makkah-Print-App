...
import { Download, FileText, Phone, User, Printer, CheckSquare, CheckCircle, AlertCircle, Clock, MapPin, DollarSign, Wallet } from "lucide-react";
...
const statusConfig: Record<string, { label: string; color: string; icon: any }> = {
    pending_review: { label: "جديد", color: "bg-orange-100 text-orange-700 border-orange-200", icon: AlertCircle },
    confirmed: { label: "مؤكد", color: "bg-blue-100 text-blue-700 border-blue-200", icon: CheckCircle },
    printing: { label: "جاري التنفيذ", color: "bg-indigo-100 text-indigo-700 border-indigo-200", icon: Printer },
    completed: { label: "جاهز للاستلام", color: "bg-green-100 text-green-700 border-green-200", icon: CheckSquare },
    out_for_delivery: { label: "جاري التوصيل", color: "bg-yellow-100 text-yellow-700 border-yellow-200", icon: CheckSquare },
    delivered: { label: "تم التسليم", color: "bg-gray-100 text-gray-700 border-gray-200", icon: CheckSquare },
};
...
export default function OrderCardLocalized({ order, updateStatus }: OrderCardProps) {
    ...
    return (
        <Card className="border-none shadow-sm hover:shadow-lg transition-all bg-white flex flex-col h-full rounded-2xl overflow-hidden ring-1 ring-slate-200 hover:ring-green-400 group" dir="rtl">
            {/* Status Badge */}
            <div className="p-1.5 absolute top-4 left-4 z-10">
                <Badge className={`${statusConfig[order.status]?.color} shadow-sm border px-3 py-1 rounded-lg font-bold flex items-center gap-1.5 text-xs tracking-wide`}>
                    <StatusIcon className="w-3.5 h-3.5 ml-1" />
                    {statusConfig[order.status]?.label || order.status}
                </Badge>
            </div>

            <CardContent className="p-6 flex-1 flex flex-col gap-5 text-right">
                {/* Header Info */}
                <div className="space-y-1 pl-16">
                    <div className="flex items-center gap-2">
                        <p className="font-black text-2xl text-slate-800 font-mono">#{order.id}</p>
                        {order.delivery === 'pickup' ? (
                            <Badge variant="outline" className="text-xs bg-slate-50 border-slate-200 text-slate-500 font-bold">استلام من الفرع</Badge>
                        ) : (
                            <Badge variant="outline" className="text-xs bg-orange-50 border-orange-200 text-orange-600 font-bold flex gap-1">
                                <MapPin className="w-3 h-3" /> توصيل
                            </Badge>
                        )}
                    </div>
                ...
                <div className="flex items-center gap-3 bg-slate-50 p-2.5 rounded-xl border border-slate-100 group-hover:bg-slate-100 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                        <p className="font-bold text-sm text-slate-900 truncate">{order.customerName || "اسم العميل غير متوفر"}</p>
                        <div className="flex items-center gap-1.5 text-xs text-slate-500">
                            <Phone className="w-3 h-3 text-slate-400 ml-1" />
                            <span className="truncate font-mono" dir="ltr">{order.customerPhone || "N/A"}</span>
                        </div>
                    </div>
                </div>

                {/* Payment Info */}
                <div className="flex justify-between items-center text-xs font-bold text-slate-500 bg-white border border-slate-100 p-2 rounded-lg">
                    <div className="flex items-center gap-1">
                        {order.paymentMethod === 'wallet' ? <Wallet className="w-3.5 h-3.5 text-green-600" /> : <DollarSign className="w-3.5 h-3.5 text-green-600" />}
                        <span>{order.paymentMethod === 'wallet' ? "محفظة إلكترونية" : "دفع كاش"}</span>
                    </div>
                    <span className="font-black text-green-700 text-lg font-mono">{order.totalPrice} ج.م</span>
                </div>
                        ...
                        {order.items.map((item, idx) => (
                            <div key={idx} className="group/file relative bg-white border border-slate-200 rounded-xl p-3 hover:border-green-300 hover:shadow-md transition-all">
                                <div className="flex items-start gap-3 mb-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-slate-400 mt-0.5 font-medium">
                                            {item.pageCount} صفحة • {item.color === 'bw' ? 'أبيض وأسود' : 'ألوان'}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-wrap gap-1.5 mb-3">
                                    ...
                                    <Badge className="bg-slate-100 text-slate-700 text-[10px] border-none font-bold px-1.5 shadow-none group-hover/file:bg-slate-200">
                                        {item.binding === 'staple' ? 'دبوس' : item.binding === 'spiral' ? 'سلك لولبي' : item.binding === 'wire' ? 'سلك معدني' : 'بدون تغليف'}
                                    </Badge>
                                </div>
                    ...
                    {isCompleted && (
                        <div className="text-center py-2 bg-green-50/50 rounded-xl border border-green-100/50">
                            <p className="text-sm font-bold text-green-700 flex items-center justify-center gap-2">
                                <CheckCircle className="w-4 h-4 animate-in fade-in zoom-in ml-2" />
                                تم الانتهاء
                            </p>
                        </div>
                    )}
